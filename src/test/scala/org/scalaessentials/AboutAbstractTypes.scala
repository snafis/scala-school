...
class AboutAbstractTypes extends FunSpec with Matchers {

  describe("A type can be declared anywhere") {
    type G = Int
    val x: G = 3
    val y: G = 5
    (x + y) should be(8)
  ...
  describe("""Abstract types are types that are declared in an abstract class, and
  ...
  describe("Abstract types can also be included in a trait") {
    trait Counter {
      type C
      def count(c: C): Int
    }
  ...
  describe("""Abstract types are also by default public, the can be locked down with an
          |  access modifier""") {
    trait Counter {
      protected type C
      def count(c: C): Int
  ...
  describe("""Perhaps the best reason for abstract types is the
          |  ability to refine types so it can make sense. The example often used
          |  is creating an Animal class, and refining the
          |  type of Food that can be eaten. This describe will also show use of a type bound
          |  as an abstract type""") {
  ...
  describe("""Abstract Types can be any type even a type with type parameter""") {
     trait Counter {
