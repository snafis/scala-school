...
//          and set a val variable to the method Value.  This is a trick to give values to each val."
  describe("Value assigns a numerical value to fields") {
...
// You can create an enumeration with your own index and your own Strings, in this describe,
// we will start with an index of one and use Greek names instead of Roman
  describe("Enumerations can set their own index and name") {
...
// Enumerations can be declared in one line if you are merely setting variables to Value
  describe("Enumeration declarations can be done on one line") {
    object Planets extends Enumeration {
  ...
  describe("Enumerations can be declared with a string value only") {
    object GreekPlanets extends Enumeration {
  ...
  describe("You can extend the Enumeration by extending the Val class.") {
