...
class AboutLiteralNumbers extends FunSpec with Matchers {
  describe("Integer Literals are 32-bit and can be created from decimal, octal, or hexadecimal") {
    val a = 2
    val b = 31
    val c = 0x30F
    val d = 077
  ...
  describe("""Long Literals are 64 bit, are specified by appending an L or l at the end;
  ...
  describe("""Float and Double Literals are IEEE 754 for specific,
  ...
  describe("""Trick: To distinguish the dot for a method invocation from the
