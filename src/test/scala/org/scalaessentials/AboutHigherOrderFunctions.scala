...
class AboutHigherOrderFunctions extends FunSpec with Matchers {
  ...
  describe("A variable referencing an anonymous function") {
    it("test"){
      val lambda = {
        x: Int => x + 1
      }
  ...
  describe("Another way for a variable to reference an anonymous function") {
    val lambda = (x: Int) => x + 1
    def result = List(1, 2, 3) map lambda
    result should be(List(2, 3, 4))
  }

  describe("Declaring a variable to reference an explicitly created function") {
  ...
  describe("""Another way for a variable to reference an explicitly created function.  In this case (Int) => Int
  ...
  describe("A closure is any function that works with it's surroundings") {
    var incrementor = 1
  ...
  describe("A function returning another function") {
  ...
  describe("function taking another function as parameter. Helps in compositioning functions") {
  ...
  describe("Currying is a technique to transform function with multiple parameters to function with one parameter") {
  ...
  describe("Currying allows you to create specialized version of generalized function") {
  ...
  describe("Using a method inside a class to create a function") {
