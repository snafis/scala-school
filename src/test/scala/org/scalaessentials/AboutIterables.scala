...
class AboutIterables extends FunSpec with Matchers {
  describe("""Iterable is a trait that has the ability to return an iterator of itself.
  ...
  describe("""`grouped` will return an fixed sized Iterable chucks of an Iterable""") {
  ...
  describe("""`sliding` will return an Iterable that shows a sliding window of an Iterable.""") {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
  ...
  describe("""`sliding` can take the size of the window as well the size of the step during each
          | iteration""") {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
  ...
  describe("""`takeRight` is the opposite of 'take' in Traversable.  It retrieves the last elements
          | of an Iterable. """) {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
    (list takeRight 3) should be(List(21, 24, 32))
  }

  describe("""`dropRight` will drop the number of elements from the right. """) {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
    (list dropRight 3) should be(List(3, 5, 9, 11, 15, 19))
  }

  describe("""`zip` will stitch two iterables into an iterable of pairs of corresponding elements
  ...
  describe("""if two Iterables aren't the same size, then `zip` will only zip what can only be paired.
  ...
  describe("""if two Iterables aren't the same size, then `zipAll` can provide fillers for what it couldn't
  ...
  describe("""`zipWithIndex` will zip an Iterable with it's integer index""") {
    val xs = List("Manny", "Moe", "Jack")
    xs.zipWithIndex should be(List(("Manny", 0), ("Moe", 1), ("Jack", 2)))
  }

  describe("""`sameElements` will return true if the two iterables have the same number of elements""") {
    val xs = List("Manny", "Moe", "Jack")
